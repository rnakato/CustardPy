...
parse_pairtools(){
    ...
    mkdir -p $pairdir $tempdir $qcdir

    if test -e "$pairdir/dedup.bwa.q$qthre.pairs.gz"; then
        echo "$pairdir/dedup.bwa.q$qthre.pairs.gz already exists. Skipping parse_pairtools."
        return
    fi
    ...
    pairtools dedup --mark-dups \
            --output $pairdir/dedup.bwa.q$qthre.pairs.gz \
            --output-stats $qcdir/pairtools_dedup.stats.txt \
            $pairdir/mapped.bwa.q$qthre.pairs.gz
